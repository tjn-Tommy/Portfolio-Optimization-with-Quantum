...
def Mps_SVD0P(T, chi_max=None, icheck=0):
    """
    (a,b,c,d) -> (a,b,l)(l,l)(l,c,d)
    对四阶张量 T 进行 SVD 分解：
        T_{a,b,c,d} = U_{a,b,l} * S_{l,l} * V_{l,c,d}
    ...
    返回:
        U : np.ndarray, 形状 (A, B, L)
        S : np.ndarray, 形状 (L, L)
        V : np.ndarray, 形状 (L, C, D)
    """
    shapeT = np.asarray(np.shape(T))
    assert len(shapeT) == 4, "T 必须是四阶张量 (A,B,C,D)"

    A, B, C, D = shapeT
    # 1️⃣ reshape 成二维矩阵
    M = np.reshape(T, [A * B, C * D])
    ...
    # 可选截断
    if chi_max is not None and chi_max < L:
        U = U[:, :chi_max]
        s = s[:chi_max]
        Vh = Vh[:chi_max, :]
        L = chi_max
    ...
    # 3️⃣ reshape 回张量形式
    U = np.reshape(U, [A, B, L])
    s = np.asarray(s).flatten()   # ✅ 确保一维
    S = np.diag(s)
    V = np.reshape(Vh, [L, C, D])

    # 4️⃣ 可选检查
    if icheck == 1:
        T_recon = np.tensordot(np.tensordot(U, S, (2, 0)), V, (2, 0))
        err = LA.norm(T - T_recon) / LA.norm(T)
        print("重构误差 =", err)
